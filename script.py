            ...
            try:
                ...
                try:
                    ...
                    if system == 'Windows':
                        ...
                        print("Speech completed using macOS 'say' fallback")
                    else:
                        # Linux — try espeak
                        subprocess.run(["espeak", text], check=True, capture_output=True)
                        print("Speech completed using espeak fallback")
...
class SignApp:
    ...
    def create_styles(self):
        ...
        # Use a theme that works well across all platforms
    ...
    def create_widgets(self):
        ...
        mono_font = 'Menlo' if platform.system() == 'Darwin' else 'Consolas' if platform.system() == 'Windows' else 'DejaVu Sans Mono'
                ...
                try:
                    ...
                    if res.multi_hand_landmarks:
                        ...
                        if xs and ys:  # Ensure we have landmarks
                            # --- FIX 1: INCREASED PADDING ---
                            # Capture forearms and space, matching dataset image proportions
                            pad = 80  
                            ...
                            if crop.size > 0 and crop.shape[0] > 10 and crop.shape[1] > 10:
                                try:
                                    # --- FIX 2: SQUARE PADDING (LETTERBOXING) ---
                                    # Prevents the hands from squishing when resized to 224x224
                                    crop_h, crop_w = crop.shape[:2]
                                    max_dim = max(crop_h, crop_w)
                                    
                                    # Create a solid black square (mimics the dark background of dataset)
                                    square_crop = np.zeros((max_dim, max_dim, 3), dtype=np.uint8)
                                    
                                    # Calculate offsets to perfectly center the hands
                                    offset_y = (max_dim - crop_h) // 2
                                    offset_x = (max_dim - crop_w) // 2
                                    
                                    # Paste the webcam crop into the center of the black square
                                    square_crop[offset_y:offset_y+crop_h, offset_x:offset_x+crop_w] = crop
                                    
                                    # Show ROI in top-right pane (Now you will see the padded square!)
                                    target_size = min(w // 2, h // 2)
                                    roi_disp = cv2.resize(square_crop, (target_size, target_size))
                                    
                                    # Prepare for model prediction
                                    roi_rgb = cv2.cvtColor(square_crop, cv2.COLOR_BGR2RGB)
                                    ...
                                    # Make prediction
                                    probs = model(x_input, training=False)[0].numpy()
